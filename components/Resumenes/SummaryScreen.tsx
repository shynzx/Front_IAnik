...
───────────────────────────────────────────────────────────── */
function SummaryCard({ summary, onView, onDelete, onCopy }: { summary: Summary; onView: () => void; onDelete: () => void; onCopy: () => void; }) {
...
/* ─────────────────────────────────────────────────────────────
   Ventana emergente para generación
───────────────────────────────────────────────────────────── */
function GenerateSummaryModal({ docs, onGenerate, onClose }: { docs: Doc[]; onGenerate: (selectedDocs: Doc[], title: string, prompt: string) => void; onClose: () => void; }) {
  ...
  return (
    <>
      ...
      <div style={{ width: "100%", margin: "0 auto", display: "flex", flexDirection: "column", gap: 20, boxSizing: "border-box" }}>
        ...
        {/* CORRECCIÓN: Agregado flexWrap: "wrap" aquí para evitar que el texto y el botón se pisen */}
